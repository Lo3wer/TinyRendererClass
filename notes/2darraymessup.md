...
void triangle(int ax, int ay, float az, int bx, int by, float bz, int cx, int cy, float cz, TGAImage& framebuffer, TGAImage& zbuffer, std::vector<std::vector<float>>& zmap, TGAColor color) {
...
std::tuple<int, int, float> project(const vec3& v) {
    int x = (v.x + 1.) * width / 2.;
...
int main(int argc, char** argv) {
    ...
    std::vector<std::vector<float>> zmap(width, std::vector<float>(height, -std::numeric_limits<float>::infinity()));
    ...
    for (int i = 0; i < model.nfaces(); i++) {
        ...
        for (int c = 0; c < 3; c++)
            rnd[c] = std::rand() % 255;
        triangle(ax, ay, az, bx, by, bz, cx, cy, cz, framebuffer, zbuffer, zmap, rnd);
    }

    std::string name = "demoncentralizedattempt2";
